#!/usr/bin/env bash
# Deploy the PB Report Link Tracker web app via clasp.
# Safe to re-run: creates the project once, then pushes + re-deploys.
set -uo pipefail
cd "$(dirname "$0")" || exit 1

TITLE="PB Report Link Tracker"
...
# Create the remote web-app project once. Protect our canonical sources in case
# clasp writes its own default appsscript.json / Code.js.
...
clasp create-deployment --description "PB link tracker" || { echo "deploy failed"; exit 1; }
...
echo "===================================================="
echo ""
echo "⚠  FINAL one-time step — authorize the mail scope:"
echo "    clasp open-script   → in the editor: Run ▸ authorizeScopes → approve the Google consent"
echo "    (clasp can't trigger that consent; without it the link can't email you.)"
