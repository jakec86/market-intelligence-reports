...
| `/dyer-pb-report` | Dyer & Dyer Volvo Price Badge Report (same flow as Nalley; $1000 threshold) | Weekly (Thu 8 AM) | Google Sheet (Dyer PB sheet) + email direct to Roman Byczek / Victor Traitel (LIVE — format approved 2026-06-12) |
...
| `/dark-prospect-report` | Show a dark prospect how their used inventory would perform on Cars.com — scrape own-site inventory + admin.cars Demand Signals churner quadrant + blended benchmark → projected VDPs/connections/$ → HTML report + deck pitch slide |
...
| Dyer PB Report | Weekly (Thu 8 AM) | launchd `com.jcrawley.dyer-pb-report` — load after first supervised run + Tableau custom view created |
...
When using Playwright MCP or Chrome DevTools MCP for browser automation:
- **Never close existing tabs or windows** — only navigate within the tab that was opened for the task, or open a new tab explicitly for that purpose
- **Never kill or close the browser** — use `browser_tabs` to check what's open and work within existing tabs where possible
- **Do not interfere with other open tabs** — if other tabs are visible (admin.cars.com, Salesforce, Gmail, etc.), leave them untouched
- **Before navigating**, use `browser_tabs list` to confirm which tab is active and avoid clobbering in-progress work in other tabs
- If you need a clean browser context for a new task, open a new tab rather than reusing a tab that may belong to another workflow
